...
from app.extensions.runtime.sync_plugin_loader import SyncPluginLoader
from app.schemas.plugin import PluginOut, PluginUpdateIn
from app.services import audit
from app.services.sync_plugins import list_sync_plugins, refresh_sync_plugins, update_sync_plugin
...
def get_sync_plugins(db: Session = Depends(get_db)):
    descriptors = _descriptor_map()
    return [_out(item, descriptors) for item in list_sync_plugins(db)]


@router.post("/refresh", response_model=list[PluginOut], dependencies=[Depends(require_permissions(PLUGIN_WRITE))])
def post_refresh_sync_plugins(request: Request, current: CurrentUser = Depends(get_current_user), db: Session = Depends(get_db)):
    items = refresh_sync_plugins(db)
    audit.write_audit_log(
        db,
        actor_user_id=current.user.id,
        action="sync_plugin.refresh",
        target_type="sync_plugin",
        target_id="*",
...
def patch_sync_plugin(request: Request, plugin_key: str, payload: PluginUpdateIn, current: CurrentUser = Depends(get_current_user), db: Session = Depends(get_db)):
    item = update_sync_plugin(db, plugin_key, **payload.model_dump(exclude_unset=True))
    audit.write_audit_log(
        db,
        actor_user_id=current.user.id,
        action="sync_plugin.update",
        target_type="sync_plugin",
